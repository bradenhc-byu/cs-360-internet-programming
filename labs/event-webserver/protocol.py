...
class Tester:
    ...
    def parse_arguments(self):
        ...
        parser.add_argument('-p', '--port', type=int, action='store', help='Port the server is running on',default=8080)
    ...
    def testForbidden(self):
        print "*** Forbidden (403) ***"
        self.open_socket()
        self.send("GET /test.txt HTTP/1.1\r\nHost: %s\r\n\r\n" % self.host)
    ...
    def testConcurrent(self):
        print "*** Partial Data and Concurrent I/O ***"
        self.open_socket()
    ...
    def testBig(self):
        ...
        self.open_socket()
        self.send("GET /largefile.txt HTTP/1.1\r\nHost: %s\r\n\r\n" % self.host)
        self.get_response([200])
        self.close_socket()
    ...
    def testRange(self):
        import requests
        print "*** HEAD ***"
        r = requests.head('http://%s:%s/largefile.txt' % (self.host,self.port))
        if r.status_code != 200:
            print "FAILED: Expected 200, got",r.status_code
        print "*** Range Request ***"
        headers = {'Range':'bytes=0-999','Accept-Encoding': 'identity'}
        r = requests.get('http://%s:%s/largefile.txt' % (self.host,self.port),headers=headers)
